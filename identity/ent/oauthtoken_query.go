...
func (_q *OAuthTokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OAuthToken, error) {
	...
	if query := _q.withApp; query != nil {
		if err := _q.loadApp(ctx, query, nodes, nil,
			func(n *OAuthToken, e *OAuthApp) { n.Edges.App = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *OAuthTokenQuery) loadApp(ctx context.Context, query *OAuthAppQuery, nodes []*OAuthToken, init func(*OAuthToken), assign func(*OAuthToken, *OAuthApp)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*OAuthToken)
	for i := range nodes {
		fk := nodes[i].AppID
...
func (_q *OAuthTokenQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(oauthtoken.Table, oauthtoken.Columns, sqlgraph.NewFieldSpec(oauthtoken.FieldID, field.TypeUUID))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withApp != nil {
			_spec.Node.AddColumnOnce(oauthtoken.FieldAppID)
		}
		if _q.withUser != nil {
