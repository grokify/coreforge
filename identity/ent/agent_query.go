...
type AgentQuery struct {
	...
	withDelegatingPrincipal *PrincipalQuery
...
func (_q *AgentQuery) Clone() *AgentQuery {
	...
	return &AgentQuery{
		...
		withDelegatingPrincipal: _q.withDelegatingPrincipal.Clone(),
...
// WithDelegatingPrincipal tells the query-builder to eager-load the nodes that are connected to
// the "delegating_principal" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AgentQuery) WithDelegatingPrincipal(opts ...func(*PrincipalQuery)) *AgentQuery {
	query := (&PrincipalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDelegatingPrincipal = query
	return _q
}
...
func (_q *AgentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Agent, error) {
	var (
		...
		loadedTypes = [2]bool{
			...
			_q.withDelegatingPrincipal != nil,
		}
	)
	...
	if query := _q.withPrincipal; query != nil {
		if err := _q.loadPrincipal(ctx, query, nodes, nil,
			func(n *Agent, e *Principal) { n.Edges.Principal = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDelegatingPrincipal; query != nil {
		if err := _q.loadDelegatingPrincipal(ctx, query, nodes, nil,
			func(n *Agent, e *Principal) { n.Edges.DelegatingPrincipal = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AgentQuery) loadPrincipal(ctx context.Context, query *PrincipalQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *Principal)) error {
...
func (_q *AgentQuery) loadDelegatingPrincipal(ctx context.Context, query *PrincipalQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *Principal)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Agent)
	for i := range nodes {
		if nodes[i].DelegatingPrincipalID == nil {
			continue
		}
		fk := *nodes[i].DelegatingPrincipalID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "delegating_principal_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AgentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPrincipal != nil {
			_spec.Node.AddColumnOnce(agent.FieldPrincipalID)
		}
		if _q.withDelegatingPrincipal != nil {
			_spec.Node.AddColumnOnce(agent.FieldDelegatingPrincipalID)
		}
	}
