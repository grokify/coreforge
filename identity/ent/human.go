...
func (*Human) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case human.FieldEmail, human.FieldGivenName, human.FieldFamilyName, human.FieldAvatarURL, human.FieldLocale, human.FieldTimezone:
...
func (_m *Human) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case human.FieldLocale:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field locale", values[i])
			} else if value.Valid {
				_m.Locale = value.String
			}
		case human.FieldTimezone:
			if value, ok := values[i].(*sql.NullString); !ok {
