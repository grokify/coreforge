...
type ApplicationQuery struct {
	...
	withIssuedTokens *PrincipalTokenQuery
...
func (_q *ApplicationQuery) Clone() *ApplicationQuery {
	...
	return &ApplicationQuery{
		...
		withIssuedTokens: _q.withIssuedTokens.Clone(),
...
// WithIssuedTokens tells the query-builder to eager-load the nodes that are connected to
// the "issued_tokens" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithIssuedTokens(opts ...func(*PrincipalTokenQuery)) *ApplicationQuery {
	query := (&PrincipalTokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withIssuedTokens = query
	return _q
}
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := _q.withPrincipal; query != nil {
		if err := _q.loadPrincipal(ctx, query, nodes, nil,
			func(n *Application, e *Principal) { n.Edges.Principal = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withIssuedTokens; query != nil {
		if err := _q.loadIssuedTokens(ctx, query, nodes,
			func(n *Application) { n.Edges.IssuedTokens = []*PrincipalToken{} },
			func(n *Application, e *PrincipalToken) { n.Edges.IssuedTokens = append(n.Edges.IssuedTokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ApplicationQuery) loadPrincipal(ctx context.Context, query *PrincipalQuery, nodes []*Application, init func(*Application), assign func(*Application, *Principal)) error {
...
func (_q *ApplicationQuery) loadIssuedTokens(ctx context.Context, query *PrincipalTokenQuery, nodes []*Application, init func(*Application), assign func(*Application, *PrincipalToken)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(principaltoken.FieldIssuedByAppID)
	}
	query.Where(predicate.PrincipalToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.IssuedTokensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.IssuedByAppID
		if fk == nil {
			return fmt.Errorf(`foreign-key "issued_by_app_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "issued_by_app_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPrincipal != nil {
			_spec.Node.AddColumnOnce(application.FieldPrincipalID)
		}
	}
