...
func (_q *InviteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invite, error) {
	...
	if query := _q.withInviter; query != nil {
		if err := _q.loadInviter(ctx, query, nodes, nil,
			func(n *Invite, e *Principal) { n.Edges.Inviter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InviteQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Invite, init func(*Invite), assign func(*Invite, *Organization)) error {
...
func (_q *InviteQuery) loadInviter(ctx context.Context, query *PrincipalQuery, nodes []*Invite, init func(*Invite), assign func(*Invite, *Principal)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Invite)
	for i := range nodes {
		fk := nodes[i].InviterPrincipalID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "inviter_principal_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InviteQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withInviter != nil {
			_spec.Node.AddColumnOnce(invite.FieldInviterPrincipalID)
		}
	}
