...
func (_q *LicenseQuery) QueryListing() *ListingQuery {
...
func (_q *LicenseQuery) QueryOrganization() *OrganizationQuery {
...
func (_q *LicenseQuery) QueryPurchaser() *PrincipalQuery {
...
func (_q *LicenseQuery) QuerySeatAssignments() *SeatAssignmentQuery {
...
func (_q *LicenseQuery) Clone() *LicenseQuery {
	...
	return &LicenseQuery{
		...
		withPurchaser:       _q.withPurchaser.Clone(),
...
func (_q *LicenseQuery) WithListing(opts ...func(*ListingQuery)) *LicenseQuery {
	query := (&ListingClient{config: _q.config}).Query()
...
func (_q *LicenseQuery) WithOrganization(opts ...func(*OrganizationQuery)) *LicenseQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LicenseQuery) WithPurchaser(opts ...func(*PrincipalQuery)) *LicenseQuery {
	query := (&PrincipalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPurchaser = query
	return _q
}
...
func (_q *LicenseQuery) WithSeatAssignments(opts ...func(*SeatAssignmentQuery)) *LicenseQuery {
	query := (&SeatAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(license.FieldListingID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *LicenseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*License, error) {
	...
	if query := _q.withListing; query != nil {
		if err := _q.loadListing(ctx, query, nodes, nil,
			func(n *License, e *Listing) { n.Edges.Listing = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOrganization; query != nil {
		if err := _q.loadOrganization(ctx, query, nodes, nil,
			func(n *License, e *Organization) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPurchaser; query != nil {
		if err := _q.loadPurchaser(ctx, query, nodes, nil,
			func(n *License, e *Principal) { n.Edges.Purchaser = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSeatAssignments; query != nil {
		if err := _q.loadSeatAssignments(ctx, query, nodes,
			func(n *License) { n.Edges.SeatAssignments = []*SeatAssignment{} },
			func(n *License, e *SeatAssignment) { n.Edges.SeatAssignments = append(n.Edges.SeatAssignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LicenseQuery) loadListing(ctx context.Context, query *ListingQuery, nodes []*License, init func(*License), assign func(*License, *Listing)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*License)
	for i := range nodes {
		fk := nodes[i].ListingID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "listing_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *LicenseQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*License, init func(*License), assign func(*License, *Organization)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*License)
	for i := range nodes {
...
func (_q *LicenseQuery) loadPurchaser(ctx context.Context, query *PrincipalQuery, nodes []*License, init func(*License), assign func(*License, *Principal)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*License)
	for i := range nodes {
		fk := nodes[i].PurchasedBy
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "purchased_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *LicenseQuery) loadSeatAssignments(ctx context.Context, query *SeatAssignmentQuery, nodes []*License, init func(*License), assign func(*License, *SeatAssignment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(seatassignment.FieldLicenseID)
	}
	query.Where(predicate.SeatAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(license.SeatAssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "license_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *LicenseQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withListing != nil {
			_spec.Node.AddColumnOnce(license.FieldListingID)
		}
		if _q.withOrganization != nil {
			_spec.Node.AddColumnOnce(license.FieldOrganizationID)
		}
		if _q.withPurchaser != nil {
			_spec.Node.AddColumnOnce(license.FieldPurchasedBy)
		}
	}
