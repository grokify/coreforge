...
func (_q *OrganizationQuery) QueryListings() *ListingQuery {
...
func (_q *OrganizationQuery) QueryLicenses() *LicenseQuery {
...
func (_q *OrganizationQuery) QuerySubscription() *SubscriptionQuery {
...
func (_q *OrganizationQuery) WithListings(opts ...func(*ListingQuery)) *OrganizationQuery {
	query := (&ListingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) WithLicenses(opts ...func(*LicenseQuery)) *OrganizationQuery {
	query := (&LicenseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) WithSubscription(opts ...func(*SubscriptionQuery)) *OrganizationQuery {
	query := (&SubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withOauthApps; query != nil {
		if err := _q.loadOauthApps(ctx, query, nodes,
			func(n *Organization) { n.Edges.OauthApps = []*OAuthApp{} },
			func(n *Organization, e *OAuthApp) { n.Edges.OauthApps = append(n.Edges.OauthApps, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withListings; query != nil {
		if err := _q.loadListings(ctx, query, nodes,
			func(n *Organization) { n.Edges.Listings = []*Listing{} },
			func(n *Organization, e *Listing) { n.Edges.Listings = append(n.Edges.Listings, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSubscription; query != nil {
		if err := _q.loadSubscription(ctx, query, nodes, nil,
			func(n *Organization, e *Subscription) { n.Edges.Subscription = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrganizationQuery) loadAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *APIKey)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.organization_api_keys
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_api_keys" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_api_keys" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *OrganizationQuery) loadOauthApps(ctx context.Context, query *OAuthAppQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OAuthApp)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(oauthapp.FieldOrganizationID)
	}
	query.Where(predicate.OAuthApp(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.OauthAppsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadServiceAccounts(ctx context.Context, query *ServiceAccountQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *ServiceAccount)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(serviceaccount.FieldOrganizationID)
	}
	query.Where(predicate.ServiceAccount(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.ServiceAccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadListings(ctx context.Context, query *ListingQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Listing)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Listing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.ListingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadLicenses(ctx context.Context, query *LicenseQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *License)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(license.FieldOrganizationID)
	}
	query.Where(predicate.License(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.LicensesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadSubscription(ctx context.Context, query *SubscriptionQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Subscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscription.FieldOrganizationID)
	}
	query.Where(predicate.Subscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.SubscriptionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
